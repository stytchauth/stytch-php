...
class PasswordsEmail
{
    ...
    * [Passwords Policy page](https://stytch.com/dashboard/password-strength-config) in the Stytch Dashboard.
    ...
    public function resetStartAsync(
        ...
        $data = is_array($request) ? $request : $request->toArray();
        $promise = $this->client->postAsync('/v1/b2b/passwords/email/reset/start', $data);
    ...
    * Note that a successful password reset by email will revoke all active sessions for the `member_id`.
    ...
    public function resetAsync(
        ...
        $data = is_array($request) ? $request : $request->toArray();
        $promise = $this->client->postAsync('/v1/b2b/passwords/email/reset', $data);
        return $promise->then(function ($response) {
            return \Stytch\B2B\Models\Passwords\Email\ResetResponse::fromArray($response);
        });
    }
    ...
    * will be deleted.
     ...
     * @return \GuzzleHttp\Promise\PromiseInterface
     */
    public function requireResetAsync(
        \Stytch\B2B\Models\Passwords\Email\RequireResetRequest|array $request,
        \Stytch\B2B\Models\Passwords\Email\RequireResetRequestOptions|array $options = [],
    ): \GuzzleHttp\Promise\PromiseInterface {
        $data = is_array($request) ? $request : $request->toArray();
        $opts = is_array($options) ? $options : $options->toArray();
        $promise = $this->client->postAsync('/v1/b2b/passwords/email/require_reset', $data, $opts);
        return $promise->then(function ($response) {
            return \Stytch\B2B\Models\Passwords\Email\RequireResetResponse::fromArray($response);
        });
    }
